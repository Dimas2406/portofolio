...
function panelMarkup(id){
  ...
  if(id==="contact")return `<p class="panel-index">04 · Contact</p><h2>Let’s work together.</h2><p class="panel-lead">I’m open to internships, junior developer roles, and collaborative web projects. If you think I could be a good fit, send me a message.</p><div class="contact-links"><a href="mailto:${data.email}"><span>${data.email}</span><b>↗</b></a>${data.socials.filter(x=>x.label!=="Email").map(x=>x.pending?`<a class="pending" aria-disabled="true"><span>${x.label}</span><b>Coming soon</b></a>`:`<a href="${x.url}" target="_blank" rel="noreferrer"><span>${x.label}</span><b>↗</b></a>`).join("")}<a href="assets/CV_Dimas_Al_Gazali_EN.pdf" download><span>Download résumé</span><b>↓</b></a></div>`;
  return "";
}
